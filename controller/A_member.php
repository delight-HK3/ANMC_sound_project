<?
    class A_member extends CI_Controller {       // a_member클래스 선언
		...
		{
			if($this->session->userdata("rank")!=1) redirect("/~sale25");
		...
		public function del(){
			...
			redirect("/~sale25/a_member/lists".$text1.$page); // 목록화면으로 돌아가기
		}
		public function add(){//사용자추가
			...
			else // 입력화면의 저장버튼 클릭한 경우
			{
				...
				redirect("/~sale25/a_member/lists".$text1.$page);//목록화면으로 이동.
			}
		}
