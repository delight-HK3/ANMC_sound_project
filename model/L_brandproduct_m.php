<?php
class L_brandproduct_m extends CI_Model{     // 모델 클래스 선언
	public function getlist($start,$limit,$no)//정보목록보기
		...
		$sql="select proproduct.*, 
			...
			proproduct.probrand_no25 like $no
			...
			// proproduct.probrand_no25와 $no 같은 값을 가져온다.
			// 제품을 가격순으로 내림차순정렬하고 값을 $limit 만큼 가져온다.
		
		return $this->db->query($sql)->result(); // 쿼리실행, 결과 리턴
        }
	public function rowcount($no){
		$sql="select * from proproduct where proproduct.probrand_no25 like $no order by name25";
		//proproduct테이블의 name25에서 progubun_no25컬럼이 $no와 같은 조건의 번호를 선택
		
		return $this->db->query($sql)->num_rows();
	}
	public function brandname($no){
		$sql="select proproduct.*, 
			probrand.name25 as brand_name25
			from proproduct 
			left join probrand on 
	...
	public function getrow($no)  {//정보추가
		$sql="select proproduct.*, 
			progubun.name25 as gubun_name25,
			probrand.name25 as brand_name25
			from proproduct 
