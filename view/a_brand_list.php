	...
	?>
		<script>
			function find_text()
			{
				if(!form1.text1.value){
					form1.action="/~sale25/a_brand/lists/page";
				}
				else{
					form1.action="/~sale25/a_brand/lists/text1/"+form1.text1.value+"/page";
				}
				form1.submit();
			}
            ...
            $(function(){
                $("#a_brand_add").click( function() { // 저장버튼 클릭시 호출
                    ...
                    $.ajax({ //a_brand 함수 호출
                        url: "/~sale25/a_brand/a_brand_insert", //a_brand_insert함수 호출
                        type: "POST",
                        data:{
                            "name":name
                        },
                ...
                $("#a_brand_edit").click( function() { // 저장버튼 클릭시 호출
                    ...
                    $.ajax({ //a_brand 함수 호출
                        url: "/~sale25/a_brand/a_brand_update", //a_brand_update함수 호출
                        type: "POST",
                        data:{
							"no":no,
                            "name":name
                ...
                $("#table_list").on("click",".a_brand_del",function() { //table_list에있는 a_brand_del을 클릭
					if(confirm("삭제할까요?")){
						no=$(this).attr("rowno");
						$.ajax({
							url: "/~sale25/a_brand/a_brand_delete", //a_brand.php에서 a_brand_delete 호출
							type: "POST",
							data: {
								"no":no //삭제버튼에서 no값 읽기
							},
		...
		<table class="table table-bordered table-sm mymargin5" id="table_list">
			...
			<tr id="rowno<?=$no?>">
				...
				<td>
					<a href="#" rowno="<?=$no?>" class="a_brand_del btn btn-sm productcolor1">삭제</a>
				</td>
			</tr>
			<?
				}
